  ...
  const AddFruits = ({
    ...
    useEffect(() => {
      ...
      const selectedItem = data?.find(
        (item) => item?.id === selectedId || item?.name === selectedId || item?.title === selectedId
      );
  
      setSelectedItem(selectedItem || null);
    }, [selectedId, data, setSelectedItem]);
  
    // Update available items based on localStorage
    useEffect(() => {
    ...
    return (
      <div>
        <Select value={selectedId || ""} onValueChange={handleSelection}>
          <SelectTrigger
            className={cn(
