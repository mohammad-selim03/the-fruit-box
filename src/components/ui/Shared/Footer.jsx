...
const Footer = () => {
  ...
  const { data, isLoading, isError } = useGetApi("system-setting", true);
  if (isLoading) {
    return (
        ...
        <div className="pt-10">
          <div className="flex items-center xl:justify-center gap-2 py-5 text-white border-t border-white/30 ">
            ...
            <p className="mt-3 text-sm xl:text-center w-2/3 lg:w-fit">
              {data?.system_name || (isError && "Not found")}{" "}
              {data?.copyright_text || (isError && "Not found")}
            </p>
          </div>
        </div>
      </Container>
